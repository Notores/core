 ...
 */
export function logWarningIfNoAuthentication(decorator, controller, func) {
 ...
 * @param func - the function name that was decorated by an authentication related decorator
 */
export function logErrorApiMetaDataDoesNotExist(decorator, controller, func) {
    if (!getConfig().authentication.enabled) {
        SystemLogger.error(`ERROR: Route does not have an HTTP handle. Use of @${decorator} in ${controller?.constructor?.name || 'Unknown'}.${func}. Try using one of ${Object.keys(HttpMethod).map((method) => `@${method}`)}`);
    }
}
