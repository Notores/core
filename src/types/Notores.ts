...
import {generateRefObject, RegisteredModule} from "../lib";

export type ClassType = { new(): Object };
// export type ClassType = { new(): Object } & typeof Object;
export type DefaultClassExport = { default: ClassType }
export type Mod = DefaultClassExport | ClassType

type SchemaObject = OpenAPIV3.SchemaObject;
type BaseSchemaObject = OpenAPIV3.BaseSchemaObject
type NonArraySchemaObjectType = OpenAPIV3.NonArraySchemaObjectType;
type ArraySchemaObject = OpenAPIV3.ArraySchemaObject;
type NonArraySchemaObject = OpenAPIV3.NonArraySchemaObject;
type ReferenceObject = OpenAPIV3.ReferenceObject;
export type NotoresSwagPropData =
    OpenAPIV3.ArraySchemaObject
    | Notores.SwagPropRefOptions
    | Notores.SwagPropRefArrayOptions
    | Notores.SwagPropOptions
    | Notores.SwagPropArrayOptions;
export type SwagEntityProperties = { [key: string]: NonArraySchemaObject }
...
export type CombinedPathOptions = PathOptions & { path: ApiPath };
...
declare module 'express' {
    ...
    export interface User extends Notores.User {

    }
}

export namespace Notores {
    ...
    export interface SwaggerRegistry extends Pick<OpenAPIV3.Document,
        'openapi'
        | 'info'
        | 'servers'
        | 'security'
    ...
    export abstract class Application extends EventEmitter {
        ...
        static async create(modules: Mod[]): Promise<Notores.Application> {
            return
        }
    ...
    export interface SwagEntityBuilder {
        name: string;
        entity: ClassType;
        required: string[];
    ...
    export interface SwagClassOptions extends BaseSchemaObject {
    }

    export interface SwagPropArrayOptions extends Omit<BaseSchemaObject, 'required'> {
        required?: boolean;
    }

    export interface SwagPropRefArrayOptions extends Omit<BaseSchemaObject, 'required'> {
        required?: boolean;
    }

    export interface SwagPropOptions extends Pick<NonArraySchemaObject,
        | 'description'
        | 'format'
        | 'default'
        | 'multipleOf'
    ...
    export interface SwagPropRefOptions {
        type: ClassType
